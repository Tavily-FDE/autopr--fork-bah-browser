...
- compare_prices: { "action": "compare_prices", "query": "..." } — for ANY price/shopping request ("preço de X", "X mais barato", "quanto custa Y", "onde comprar Z"): scrapes Google Shopping (which aggregates Mercado Livre, Amazon, Magalu, KaBuM…) and, when a Tavily API key is configured, also queries Tavily for additional price results — then merges and opens a price-sorted comparison table. Use this as the first action for generic price requests — but if the user named a specific store/site or gave a URL, navigate there and use its own search instead of this.
